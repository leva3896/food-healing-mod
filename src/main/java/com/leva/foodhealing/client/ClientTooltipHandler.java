        ...
        if (stack.getItem().getFoodProperties(stack, player) != null) {
            ...
            if (foodId != null) {
                player.getCapability(FoodDiversityProvider.FOOD_DIVERSITY).ifPresent(data -> {
                    ...
                    if (eaten) {
                        event.getToolTip().add(Component.translatable("tooltip.foodhealing.eaten").withStyle(ChatFormatting.DARK_GREEN));
                    } else {
                        event.getToolTip().add(Component.translatable("tooltip.foodhealing.not_eaten").withStyle(ChatFormatting.GRAY));
                    }
                });
            }
        }
